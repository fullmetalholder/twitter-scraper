...
async function requestApiPostForm(url, auth, body, platform = new Platform(), bearerTokenOverride) {
  const headers = new Headers();
...
const FRIENDSHIPS_CREATE_URL = "https://x.com/i/api/1.1/friendships/create.json";
...
async function followUser(auth, username) {
  if (!await auth.isLoggedIn()) {
  ...
  const res = await requestApiPostForm(
...
const GRAPHQL_CREATE_TWEET = "https://x.com/i/api/graphql/a1p9RWpkYKBjWv_I3WzS-A/CreateTweet";
const GRAPHQL_CREATE_RETWEET = "https://x.com/i/api/graphql/ojPdsZsimiJrUGLR1sjUtA/CreateRetweet";
const GRAPHQL_DELETE_RETWEET = "https://x.com/i/api/graphql/iQtK4dl5hBmXewYZuEOKVw/DeleteRetweet";
const GRAPHQL_FAVORITE_TWEET = "https://x.com/i/api/graphql/lI07N6Otwv1PhnEgXILM7A/FavoriteTweet";
const GRAPHQL_UNFAVORITE_TWEET = "https://x.com/i/api/graphql/ZYKSe-w7KEslx3JhSIk5LA/UnfavoriteTweet";
...
async function getMediaUploadHeaders(auth, url) {
  const uploadUrl = "https://upload.x.com";
  const cookies = await auth.cookieJar().getCookies(uploadUrl);
  const xCsrfToken = cookies.find((cookie) => cookie.key === "ct0");
  const bearerToken = auth.bearerToken || "AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA";
...
async function uploadMedia(auth, mediaData) {
  ...
  const headers = await getMediaUploadHeaders(auth);
  ...
  while (offset < totalBytes) {
    ...
    const appendHeaders = await getMediaUploadHeaders(auth);
  ...
  const finalizeHeaders = await getMediaUploadHeaders(auth);
...
async function waitForMediaProcessing(auth, mediaId) {
  while (true) {
    ...
    const waitTime = (statusResult.processing_info.check_after_secs ?? 5) * 1e3;
    await new Promise((resolve) => setTimeout(resolve, waitTime));
  }
}
async function createTweet(auth, text, options) {
...
async function retweet(auth, tweetId) {
...
async function unretweet(auth, tweetId) {
...
async function likeTweet(auth, tweetId) {
...
async function unlikeTweet(auth, tweetId) {
...
class Scraper {
   ...
   * @returns The response from Twitter.
   */
  async sendQuoteTweet(text, quotedTweetId, mediaData) {
    const options = {
...
export { ApiError, AuthenticationError, ErrorRateLimitStrategy, Scraper, SearchMode, WaitingRateLimitStrategy, createTweet, fetchProfileFollowers, fetchProfileFollowing, followUser, getFollowers, getFollowing, likeTweet, retweet, unlikeTweet, unretweet, uploadMedia };
